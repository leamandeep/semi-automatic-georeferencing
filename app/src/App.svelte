...
<script>
  ...
  function checkAndAddPair() {
    if (selectedRaw && selectedRef) {
      ...
      const exists = pairs.some(p => p[0] === selectedRaw && p[1] === selectedRef);
  ...
  async function applyTransformation() {
    ...
    try {
      const blob = await api.applyTransform(sessionId, rawKeyCol, refKeyCol, pairs);
...
<main class="flex flex-col items-center gap-5 my-10 px-4">
  ...
  {#if step === 1}
    <div class="flex gap-5">
      <fieldset class="fieldset border p-4 rounded">
        <legend class="fieldset-legend font-semibold px-2">Pick a Raw file</legend>
        <input 
          type="file" 
          name="raw" 
      ...
      <fieldset class="fieldset border p-4 rounded">
        <legend class="fieldset-legend font-semibold px-2">Pick a Ref file</legend>
        <input 
          type="file" 
          name="ref" 
          class="file-input  w-full max-w-xs" 
  ...
  {#if step === 3}
    <div class="w-full max-w-7xl">
      <h2 class="text-xl font-semibold mb-4">Step 1: Select Matching Plot Numbers</h2>
        ...
        <div class="card-body">
          ...
          {:else}
            <p class="text-sm opacity-70">
              Click on matching plots in both maps to add control pairs. At least 3 pairs required.
            </p>
          {/if}
